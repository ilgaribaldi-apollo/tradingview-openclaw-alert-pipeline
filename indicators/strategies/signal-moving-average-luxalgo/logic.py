...
def _signal_ma(close: pd.Series, length: int = 50) -> tuple[pd.Series, pd.Series]:
    ...
    r = close.rolling(length, min_periods=length).corr(bar_index)
    r2 = r.pow(2)
    ...
    return pd.Series(ma_vals, index=close.index, name="signal_ma"), pd.Series(os_vals, index=close.index, name="os")


def generate_signals(df: pd.DataFrame) -> pd.DataFrame:
    df = df.copy()
    df["signal_ma"], df["os_state"] = _signal_ma(df["close"], 50)
    prev_close = df["close"].shift(1)
    prev_ma = df["signal_ma"].shift(1)
    df["entry"] = (prev_close <= prev_ma) & (df["close"] > df["signal_ma"])
    df["exit"] = (prev_close >= prev_ma) & (df["close"] < df["signal_ma"])
    df[["entry", "exit"]] = df[["entry", "exit"]].fillna(False)
    return df
